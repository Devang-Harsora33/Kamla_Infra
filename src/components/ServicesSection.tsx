  ...
  return (
    <section id="services" className="py-20 lg:py-28 bg-slate-50 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredServices.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="bg-white rounded-2xl p-7 sm:p-8 border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group"
              >
                <div>
                  {/* Card Top */}
                  <div className="flex items-start justify-between gap-4 mb-6">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-white ${
                        isOrangeAccent ? 'bg-[#E85D04]' : 'bg-[#0B3B60]'
                      }`}
                    >
                  ...
                  {/* Title & Tagline */}
                  <h3 className="font-heading text-2xl font-bold text-slate-900 mb-1 group-hover:text-[#0B3B60] transition-colors">
                    {service.title}
                  </h3>
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-lg font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                    isOrangeAccent
                      ? 'bg-slate-900 hover:bg-[#0B3B60] text-white'
                      : 'bg-[#0B3B60] hover:bg-[#07253D] text-white'
                  }`}
                >
        ...
        <div className="mt-12 p-6 sm:p-8 rounded-2xl bg-[#07253D] text-white flex flex-col md:flex-row items-center justify-between gap-6">
